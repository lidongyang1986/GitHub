...
function random_write_4k_iops()
{
	echo -e "`date +\%Y-\%m-\%d_\%H-\%M-\%S`: Attempting 4k random write 1 min..."
	fio_opt_spec="--blocksize=4k --rw=randwrite  --filename=$1 --name=test --iodepth=256 --runtime=60 --time_based"
	
	fio_opt="$fio_opt_base $fio_opt_spec"
	echo -ne "$fio_opt;\n"
	fio $fio_opt
	if [ $? = 0 ]; then
		echo -e "\033[32m`date +\%Y-\%m-\%d_\%H-\%M-\%S`: 4k random write 1 min successful!!!\033[0m"
	else
		echo -e "\033[1;31m`date +\%Y-\%m-\%d_\%H-\%M-\%S`: 4k random write 1 min fail!!!\033[0m"
		exit 1
	fi
}
