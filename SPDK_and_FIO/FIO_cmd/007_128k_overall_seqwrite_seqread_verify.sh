#!/bin/bash

function identify_lbaf()
{
	lbaf=$(nvme id-ns $1 | awk '/lbaf/&&/in use/''{print $5}')
	if [ "$lbaf" = "lbads:9" ]; then
		verify_interval_var="512"
	elif [ "$lbaf" = "lbads:12" ]; then
		verify_interval_var="4096"
	fi
}

function identify_disk()
{
	disks=$(fdisk -l | awk '{if($2~/nvme/) {split($2,a,":"); disk=disk" "a[1];}} END {print disk}')
	disksA=$(iostat | awk '{if($1~/nvme/) {$1="/dev/"$1;disk=disk" "$1;}} END {print disk}')
	if [[ "${disks[@]}" =~ "$1" ]]; then
		echo "Make sure to run fio on:" $1
		if [[ "${disksA[@]}" =~ "$1" ]]; then
			direct="1"
		else
			direct="1"
		fi
	else
		echo -e "\033[1;31mWarnning:the device under test $1 is absent ...\033[0m"
		exit 2
	fi
}
function seq_write_overall()
{
	## Unit:KiB
	blocksize_ver="128"
	## capacity,B
	DiskSize_B=`sudo fdisk -l $1 2>/dev/null | awk '/Disk/&&/bytes/''{print $5}'`
	Cnt4KiB=`sudo fdisk -l $1 2>/dev/null | awk '/Disk/&&/bytes/''{print int($5/4096)}'`
	Mul4kBS=$(($blocksize_ver/4))
	CntBsAlig=$(($Cnt4KiB/$Mul4kBS))
	DiskSize_BSKiB=$(($CntBsAlig*$blocksize_ver*1024))
	Cnt4KibBsUnalig=$(($Cnt4KiB%$Mul4kBS))
	DiskSize_BsUnalig=$(($Cnt4KibBsUnalig*4096))

	echo -e "\033[32m`date +\%Y-\%m-\%d_\%H-\%M-\%S`: the capacity of $1 is $DiskSize_B Bytes = $CntBsAlig * ${blocksize_ver}k + $Cnt4KibBsUnalig * 4k = $DiskSize_BSKiB + $DiskSize_BsUnalig\033[0m"
	echo -e "`date +\%Y-\%m-\%d_\%H-\%M-\%S`: Attempting ${blocksize_ver}k sequential write on the ${blocksize_ver}k aligned of entire device. This may take a while..."
	fio_opt_spec="--blocksize=${blocksize_ver}k --rw=write  --filename=$1 --name=test --iodepth=256 --size=$DiskSize_BSKiB"
	
	fio_opt="$fio_opt_base $fio_opt_spec $fio_opt_varify1"
	echo -ne "$fio_opt;\n"
	fio $fio_opt
	if [ $? = 0 ]; then
		echo -e "\033[32m`date +\%Y-\%m-\%d_\%H-\%M-\%S`: ${blocksize_ver}k sequential write on entire device successful...\033[0m"
	else
		echo -e "\033[1;31m`date +\%Y-\%m-\%d_\%H-\%M-\%S`: ${blocksize_ver}k sequential write on entire device fail...\033[0m"
		exit 1
	fi
	
	if [ $DiskSize_BsUnalig -eq 0 ]; then
		return 0
	fi
	echo -e "`date +\%Y-\%m-\%d_\%H-\%M-\%S`: Attempting 4k sequential write on the ${blocksize_ver}k unaligned of entire device. This may take a while..."
	fio_opt_spec="--blocksize=4k --rw=write  --filename=$1 --name=test --iodepth=256 --offset=$DiskSize_BSKiB --size=$DiskSize_BsUnalig"
	fio_opt="$fio_opt_base $fio_opt_spec $fio_opt_varify1"
	echo -ne "$fio_opt;\n"
	fio $fio_opt
	if [ $? = 0 ]; then
		echo -e "\033[32m`date +\%Y-\%m-\%d_\%H-\%M-\%S`: 4k sequential write on entire device successful...\033[0m"
	else
		echo -e "\033[1;31m`date +\%Y-\%m-\%d_\%H-\%M-\%S`: 4k sequential write on entire device fail...\033[0m"
		exit 1
	fi
}
function seq_read_overall()
{
	## Unit:KiB
	blocksize_ver="128"
	## capacity,B
	DiskSize_B=`sudo fdisk -l $1 2>/dev/null | awk '/Disk/&&/bytes/''{print $5}'`
	Cnt4KiB=`sudo fdisk -l $1 2>/dev/null | awk '/Disk/&&/bytes/''{print int($5/4096)}'`
	Mul4kBS=$(($blocksize_ver/4))
	CntBsAlig=$(($Cnt4KiB/$Mul4kBS))
	DiskSize_BSKiB=$(($CntBsAlig*$blocksize_ver*1024))
	Cnt4KibBsUnalig=$(($Cnt4KiB%$Mul4kBS))
	DiskSize_BsUnalig=$(($Cnt4KibBsUnalig*4096))

	echo -e "\033[32m`date +\%Y-\%m-\%d_\%H-\%M-\%S`: the capacity of $1 is $DiskSize_B Bytes = $CntBsAlig * ${blocksize_ver}k + $Cnt4KibBsUnalig * 4k = $DiskSize_BSKiB + $DiskSize_BsUnalig\033[0m"
	echo -e "`date +\%Y-\%m-\%d_\%H-\%M-\%S`: Attempting ${blocksize_ver}k sequential read on the ${blocksize_ver}k aligned of entire device. This may take a while..."
	fio_opt_spec="--blocksize=${blocksize_ver}k --rw=read  --filename=$1 --name=test --iodepth=256 --size=$DiskSize_BSKiB"
	fio_opt="$fio_opt_base $fio_opt_spec $fio_opt_varify2"
	echo -ne "$fio_opt;\n"
	fio $fio_opt
	if [ $? = 0 ]; then
		echo -e "\033[32m`date +\%Y-\%m-\%d_\%H-\%M-\%S`: ${blocksize_ver}k sequential read on entire device successful!!!\033[0m"
	else
		echo -e "\033[1;31m`date +\%Y-\%m-\%d_\%H-\%M-\%S`: ${blocksize_ver}k sequential read on entire device fail!!!\033[0m"
		exit 1
	fi
	
	if [ $DiskSize_BsUnalig -eq 0 ]; then
		return 0
	fi
	echo -e "`date +\%Y-\%m-\%d_\%H-\%M-\%S`: Attempting 4k sequential read on the ${blocksize_ver}k unaligned of entire device. This may take a while..."
	fio_opt_spec="--blocksize=4k --rw=read  --filename=$1 --name=test --iodepth=256 --offset=$DiskSize_BSKiB --size=$DiskSize_BsUnalig"
	fio_opt="$fio_opt_base $fio_opt_spec $fio_opt_varify2"
	echo -ne "$fio_opt;\n"
	fio $fio_opt
	if [ $? = 0 ]; then
		echo -e "\033[32m`date +\%Y-\%m-\%d_\%H-\%M-\%S`: 4k sequential read on entire device successful...\033[0m"
	else
		echo -e "\033[1;31m`date +\%Y-\%m-\%d_\%H-\%M-\%S`: 4k sequential read on entire device fail...\033[0m"
		exit 1
	fi
}

if [[ $# -eq 0 ]]; then
	dev_var="/dev/nvme0n1"
	identify_disk $dev_var
	identify_lbaf $dev_var
	verify_backlog_var=`expr $RANDOM % 5 + 1`
	verify_backlog_batch_var=`expr $RANDOM % 10 + 1`
	fio_opt_base="--ioengine=sync --direct=$direct --thread=1 --numjobs=1 --group_reporting"
	fio_opt_varify1="--do_verify=1 --verify_interval=$verify_interval_var --verify_backlog=$verify_backlog_var --verify_backlog_batch=$verify_backlog_batch_var --verify_fatal=1 --verify_dump=1 --verify_pattern="
	fio_opt_varify2="--do_verify=1 --verify_interval=$verify_interval_var --verify_fatal=1 --verify_dump=1 --verify_pattern="
fi

seq_write_overall $dev_var
seq_read_overall $dev_var


