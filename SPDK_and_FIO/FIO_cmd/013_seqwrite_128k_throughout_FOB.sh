...
function seq_write_128k_throughout()
{
	echo -e "`date +\%Y-\%m-\%d_\%H-\%M-\%S`: Attempting 128k sequential write 1 min..."
	fio_opt_spec="--blocksize=128k --rw=write  --filename=$1 --name=test --iodepth=256 --runtime=60 --time_based"
	
	fio_opt="$fio_opt_base $fio_opt_spec"
	echo -ne "$fio_opt;\n"
	fio $fio_opt
	if [ $? = 0 ]; then
		echo -e "\033[32m`date +\%Y-\%m-\%d_\%H-\%M-\%S`: 128k sequential write 1 min successful!!!\033[0m"
	else
		echo -e "\033[1;31m`date +\%Y-\%m-\%d_\%H-\%M-\%S`: 128k sequential write 1 min fail!!!\033[0m"
		exit 1
	fi
}
...
seq_write_128k_throughout $dev_var
