...
function seq_read_overall()
{
	...
	echo -e "\033[32m`date +\%Y-\%m-\%d_\%H-\%M-\%S`: the capacity of $1 is $DiskSize_B Bytes = $CntBsAlig * ${blocksize_ver}k + $Cnt4KibBsUnalig * 4k = $DiskSize_BSKiB + $DiskSize_BsUnalig\033[0m"
	echo -e "`date +\%Y-\%m-\%d_\%H-\%M-\%S`: Attempting ${blocksize_ver}k sequential read (sequential read on the ${blocksize_ver}k aligned of entire device). This may take a while..."
